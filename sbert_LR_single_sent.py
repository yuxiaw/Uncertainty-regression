...
edit date: 2021-07-06
# add experiment for SA rating: single sentence rating
"""
import os
...
from util_data import Totorch, get_labels, eval_correlation
from sbert_embedding import load_single_sent_features
...
def run_blr(args, pd_preds=False):
    # load training data
    data_list = load_single_sent_features(dataset_name=args.dataset, returnFL=True)
    train_e, train_labels = data_list[0]
    # generate features as torch.tensor
    x_data, y_data = Totorch(train_e, train_labels)
    try: 
        dev_e, dev_labels = data_list[1]
        x_data_dev, y_data_dev = Totorch(dev_e, dev_labels)
    except IndexError:
        x_data_dev = None
        print("There isn't dev set.")
    try: 
        test_e, test_labels = data_list[2]  
        x_data_test, y_data_test = Totorch(test_e, test_labels)
    except IndexError:
        x_data_test = None
